...
void test_pixel_inside_colored(void) {
  ...
  draw_triangle_pixels_tiled(depth, fb, NULL, W, &st, 0, 0, W - 1, H - 1);
...
void test_depth_test_rejects_farther_triangle(void) {
  ...
  draw_triangle_pixels_tiled(depth, fb, NULL, W, &near, 0, 0, W - 1, H - 1);

  screen_tri_t far = base_tri(COL_GREEN);
  far.inv_z[0] = far.inv_z[1] = far.inv_z[2] = 0.25f; /* z=4, farther */
  draw_triangle_pixels_tiled(depth, fb, NULL, W, &far, 0, 0, W - 1, H - 1);
...
void test_clip_excludes_pixel_left_of_bound(void) {
  ...
  draw_triangle_pixels_tiled(depth, fb, NULL, W, &st, 4, 0, W - 1, H - 1);
  TEST_ASSERT_EQUAL_UINT8(0, fb[2 * W + 2].r);
}
...
void test_clip_excludes_pixel_above_bound(void) {
  ...
  draw_triangle_pixels_tiled(depth, fb, NULL, W, &st, 0, 4, W - 1, H - 1);
  TEST_ASSERT_EQUAL_UINT8(0, fb[2 * W + 2].r);
}
...
/* ---- fetch_pixel_color: texture sampling ---- */
...
void test_idbuffer_written_for_covered_pixel(void) {
  ...
  screen_tri_t st = base_tri(COL_RED);
  st.instance_id = 7;
  draw_triangle_pixels_tiled(depth, fb, idbuf, W, &st, 0, 0, W - 1, H - 1);
