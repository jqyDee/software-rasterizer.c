...
INTERNAL void triangle_coordinates_to_tile_coordinates(
    ...
    int *ty1) {
  // convert tri bbox (absolute framebuffer coords) to tile coordinates
  // relative to this viewport's own tile grid
  *tx0 = (screen_triangle->bx0 - vp_x0) / TILE_SIZE;
  *ty0 = (screen_triangle->by0 - vp_y0) / TILE_SIZE;
  *tx1 = (screen_triangle->bx1 - vp_x0) / TILE_SIZE,
  *ty1 = (screen_triangle->by1 - vp_y0) / TILE_SIZE;
...
void compute_triangles_per_tile(
  ...
  for (int screen_triangle_id = 0; screen_triangle_id < screen_triangles_count;
    ...
    triangle_coordinates_to_tile_coordinates(
        &screen_triangles[screen_triangle_id], vp_x0, vp_y0, tiles_x, tiles_y,
        &tx0, &tx1, &ty0, &ty1);
...
void tile_coordinates_to_triangle_coordinates(const int tile_index,
  ...
  *x0 = vp_x0 + x * TILE_SIZE;
  *y0 = vp_y0 + y * TILE_SIZE;
  *x1 = *x0 + TILE_SIZE - 1;
